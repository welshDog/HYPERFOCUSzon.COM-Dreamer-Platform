...
class Web3NewsEnhancementEngine:
    ...
    async def fetch_defi_data(self) -> List[DeFiData]:
        ...
        logger.info("✅ Fetched %s DeFi protocols", len(defi_data))

        except Exception as e:
        logger.error("❌ DeFi data fetch error: %s", e)
    ...
    async def fetch_nft_data(self) -> List[NFTCollection]:
        ...
        logger.info("✅ Fetched %s NFT collections", len(nft_data))

        except Exception as e:
        logger.error("❌ NFT data fetch error: %s", e)
