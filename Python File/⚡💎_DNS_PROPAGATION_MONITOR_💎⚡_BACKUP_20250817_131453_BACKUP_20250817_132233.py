...
def check_dns_resolution():
    ...
    try:
        ...
        if "can't find" in result.stdout or "Non-existent" in result.stdout:
            ...
            return True, "✅ DNS record found and pointing correctly"
        else:
            return False, f"⚠️  DNS found but not pointing correctly: {result.stdout.strip()}"
    ...
    try:
        ...
        if response.status_code == 200:
            if "SUPPORT THE HYPERFOCUS EMPIRE" in response.text:
                return True, "🎉 DONATION PORTAL LIVE! Custom domain working perfectly!"
            else:
                return True, "✅ Site responding but content may not be ready"
        elif response.status_code == 404:
            return False, "⚠️  GitHub Pages not ready (404 error)"
        else:
            return False, f"⚠️  Site responding with status {response.status_code}"
    except requests.exceptions.SSLError:
        return False, "🔒 SSL certificate not ready yet"
    except requests.exceptions.ConnectionError:
        return False, "❌ Connection failed - DNS not propagated yet"
    except Exception as e:
    ...
    try:
        ...
        with socket.create_connection(('support.hyperfocuszone.com', 443), timeout=10) as sock:
            with context.wrap_socket(sock, server_hostname='support.hyperfocuszone.com') as ssock:
                ...
                return True, f"🔐 SSL Certificate ready! Issued by: {cert.get('issuer', 'Unknown')}"
    except Exception as e:
        return False, f"🔒 SSL not ready: {str(e)}"

def monitor_deployment():
    """Main monitoring function"""
    print("🚀 HYPERFOCUS EMPIRE DNS PROPAGATION MONITOR 🚀")
    print("=" * 60)
    ...
    while attempt < max_attempts and not (dns_ready and github_ready and ssl_ready):
        ...
        if dns_ready:
            ...
            print("GitHub Pages: ⏳ Waiting for DNS to resolve first")
            print("SSL: ⏳ Waiting for DNS to resolve first")
        ...
        if dns_ready and github_ready and ssl_ready:
            print("\n" + "🎉" * 20)
            print("🚀 DONATION PORTAL IS LIVE! 🚀")
            print("💎⚡ https://support.hyperfocuszone.com ⚡💎")
            print("🎉" * 20)
            break
    ...
    if not (dns_ready and github_ready and ssl_ready):
        ...
        print("💡 This is normal - DNS can take up to 24-48 hours to fully propagate.")
        print("🔄 Continue checking manually or run this script again later.")
