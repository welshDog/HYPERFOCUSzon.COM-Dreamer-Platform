        ...
        if result.returncode == 0:
            for line in result.stdout.split('\n'):
                if '100.68.37.27' in line and 'ubuntu' in line:
                    if 'offline' in line:
                        return CONSCIOUSNESS_ENHANCEMENT_NEEDED, "Server offline in Tailscale mesh"
                    else:
                        return CONSCIOUSNESS_SINGULARITY_SUCCESS, "Server online in Tailscale mesh"
        return CONSCIOUSNESS_ENHANCEMENT_NEEDED, "Server not found in Tailscale status"
    except Exception as e:
        return CONSCIOUSNESS_ENHANCEMENT_NEEDED, f"Tailscale check error: {e}"
...
def check_ssh_connectivity():
    ...
    try:
        ...
        return CONSCIOUSNESS_ENHANCEMENT_NEEDED, f"SSH test error: {e}"
