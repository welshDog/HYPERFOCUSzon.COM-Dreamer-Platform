...
class LegendaryGrafanaDashboardDeployer:
    ...
    def validate_connection(self) -> bool:
        ...
        try:
            logger.info("🌌 🔍 Validating connection to Grafana Cloud...")
    ...
    def upload_dashboard(self, dashboard_payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            logger.info("🌌 🚀 Uploading dashboard to Grafana Cloud...")
...
🏛️ Examples:
  ...
  python grafana_dashboard_deployer.py --dashboard dashboard-final.json --url https://custom.grafana.net --token your_token
    ...
    if not service_token:
        ...
        if not service_token:
            logger.info("🌌 ❌ No service account token provided!")
            logger.info("🌌 Use --token parameter or set GRAFANA_SERVICE_ACCOUNT_TOKEN environment variable")
            sys.exit(1)
...
⚡ Status: Initiating deployment...
    ...
    if not deployer.validate_connection():
        logger.info("🌌 ❌ Failed to connect to Grafana Cloud. Aborting deployment.")
        sys.exit(1)

    # Step 2: Load dashboard
    dashboard_data = deployer.load_dashboard_json(args.dashboard)
    if not dashboard_data:
        logger.info("🌌 ❌ Failed to load dashboard JSON. Aborting deployment.")
        sys.exit(1)
    ...
    if not result:
        logger.info("🌌 ❌ Dashboard upload failed. Aborting.")
        sys.exit(1)
...
LEGENDARY EMPIRE COST TRACKING: OPERATIONAL! 🎊
        ...
        logger.info("🌌 ⚠️ Dashboard uploaded but verification failed. Check manually.")
        sys.exit(1)
