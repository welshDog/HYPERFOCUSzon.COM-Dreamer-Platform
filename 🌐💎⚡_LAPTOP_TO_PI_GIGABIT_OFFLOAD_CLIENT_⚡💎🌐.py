...
class GigabitPiOffloadClient:
    ...
    async def __aenter__(self):
        ...
        self.logger.info(f"🚀 Gigabit Pi offload client initialized (Pi: {self.pi_ip})")
        return self
        ...
        try:
            async with self.session.get(f"{self.pi_base_url}/health") as response:
                if response.status == 200:
                    ...
                    self.logger.info(f"✅ Pi connection validated: {data.get('status', 'unknown')}")
                    return True
                else:
                    self.logger.error(f"❌ Pi health check failed: HTTP {response.status}")
                    return False
        except Exception as e:
            self.logger.error(f"❌ Pi connection validation failed: {e}")
            return False
    ...
    async def _task_worker(self, worker_id: str):
        ...
        while True:
            try:
                ...
                self.logger.info(f"🔚 Task worker {worker_id} cancelled")
                break
            except Exception as e:
                self.logger.error(f"❌ Task worker {worker_id} error: {e}")
